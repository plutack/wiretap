...
const SHELL_OPTIONS = [
  ...
  { value: "bash", label: "bash" }, { value: "fish", label: "fish" },
  { value: "powershell", label: "powershell" }, { value: "gitbash", label: "git bash" },
];
...
function SettingsNav({ active, onChange }) {
  return html`<nav class="settings-rail" aria-label="Settings sections">
    ...
    ${NAV_ITEMS.filter((item) => !item.server).map((item) => html`<button class=${active === item.id ? "active" : ""} onClick=${() => onChange(item.id)}>
      <strong>${item.label}</strong><span>${item.hint}</span>
    </button>`)}
    <div class="settings-rail-label settings-rail-server">Server administration</div>
    ${NAV_ITEMS.filter((item) => item.server).map((item) => html`<button class=${active === item.id ? "active" : ""} onClick=${() => onChange(item.id)}>
      <strong>${item.label}</strong><span>${item.hint}</span>
    </button>`)}
  </nav>`;
...
function SettingsCard({ title, hint, children }) {
  return html`<section class="settings-card"><header><h3>${title}</h3>${hint ? html`<p>${hint}</p>` : null}</header><div class="settings-card-body">${children}</div></section>`;
}
...
export function Settings({ onToast, onSaved }) {
  ...
  const load = async () => {
    try {
      ...
      const next = {
        relay_url: s.relay_url || "", forward_url: s.forward_url || "", store_path: s.store_path || "",
        tui_theme: s.tui_theme || "dark", native_titlebar: s.native_titlebar || "auto",
        proxy_addr: s.proxy_addr || "", local_api_addr: s.local_api_addr || "", shell: s.shell || "",
      };
      setView(s); setForm(next); setSavedForm(next);
      setReg((current) => ({ ...current, url: current.url || s.relay_url || "" }));
    } catch (e) { onToast("load settings: " + e); }
  };
  useEffect(() => { load(); }, []);
  if (!view || !form) return html`<div class="p-6 text-sm text-neutral-500">Loading settings...</div>`;

  const refreshView = async () => {
    try { setView(await api.getSettings()); onSaved && onSaved(); }
    catch (e) { onToast("refresh settings: " + e); }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      setView(s); setSavedForm({ ...form }); onToast("Settings saved"); onSaved && onSaved();
    } catch (e) { onToast("save settings: " + e, 5000); }
  ...
  const register = async () => {
    setRegistering(true);
    try {
      const result = await api.registerRelay({ relay_url: reg.url, admin_token: reg.token,
        projects: reg.projects.split(",").map((item) => item.trim()).filter(Boolean), display_name: reg.name });
      setReg((current) => ({ ...current, token: "" })); onToast(`Registered as ${result.client_id}`);
      await load(); onSaved && onSaved();
    } catch (e) { onToast("register: " + e, 6000); }
    finally { setRegistering(false); }
  };
  const addProject = async () => {
    const project = projectDraft.trim().replace(/^\/+|\/+$/g, ""); if (!project) return;
    setProjectBusy(true);
    try { const s = await api.addRelayProject(project); setView(s); setProjectDraft(""); onToast(`Added project ${project}`); onSaved && onSaved(); }
    catch (e) { onToast("add project: " + e, 6000); }
  ...
  return html`<div class="settings-workspace">
    ...
    <main class="settings-content">
      ${section === "relay" ? html`<${RelayAdmin} defaultURL=${view.relay_url} localClientID=${view.client_id} onToast=${onToast} onChanged=${refreshView} />` : null}
      ${section === "connection" ? html`
        <${PageHeader} title="Relay connection" hint="Connect this desktop once, then manage its webhook paths without rotating credentials." />
        ...
        ${!view.registered ? html`<${SettingsCard} title="Register this desktop" hint="Creates one identity. Initial projects are optional and can be added later without registering again.">
      ...
      ` : null}
      ${section !== "relay" ? html`<div class="settings-savebar"><div><strong>${dirty ? "Unsaved changes" : "All changes saved"}</strong><span>${view.config_path}</span></div>
        <${Button} variant="primary" disabled=${saving || !dirty} onClick=${save}>${saving ? "Saving..." : "Save changes"}</></div>` : null}
    </main>
  </div>`;
}
