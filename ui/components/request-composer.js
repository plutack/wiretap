...
export function RequestComposer({ initialRequest, onSend, onToast }) {
  ...
  return html`<div class="composer-shell">
    <header class="composer-header">
      <div>
        ...
        <p>Author a request, import JSON, or continue from a captured delivery.</p>
      </div>
      <div class="flex gap-2">
        <input ref=${fileRef} type="file" accept="application/json,.json" class="hidden" onChange=${importFile} />
        <${Button} onClick=${() => fileRef.current?.click()}>Import JSON</>
        <${Button} onClick=${() => applyImported(JSON.stringify({ hello: "world" }))}>JSON example</>
      </div>
    </header>
    ...
    <section class="composer-grid">
      <div class="composer-panel">
        ...
        <label class="composer-label">Headers <span>JSON object; values may be strings or arrays</span></label>
        ...
        <label class="composer-label">Body <span>plain text or JSON</span></label>
      ...
      <div class="composer-panel composer-response">
        ...
        ` : html`<div class="composer-response-empty"><span>↗</span><strong>No response yet</strong><p>Send the request to inspect status, headers, body, and timing.</p></div>`}
      </div>
    </section>
  </div>`;
}
